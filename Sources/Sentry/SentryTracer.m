...
- (void)finishInternal
{
    ...
    @synchronized(_children) {
        if ([self hasIdleTimeout] && _children.count == 0) {
            SENTRY_LOG_DEBUG(
                @"Was waiting for timeout for UI event trace and no children to wait for.");
            if ([self didAutomaticTraceExceedMaxDuration]) {
                SENTRY_LOG_DEBUG(@"Transaction exceeded max duration.");
                return;
            }
#if SENTRY_TARGET_PROFILING_SUPPORTED
 ...
 */
- (BOOL)didAutomaticTraceExceedMaxDuration
{
